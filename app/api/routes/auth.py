...
class SignUpRequest(BaseModel):
    ...
    def validate_password(cls, value):
        ...
        if len(value) < 8:
            raise HTTPException(
                status_code=400, detail="비밀번호는 최소 8자리 이상이어야 합니다."
            )
        if (
    ...
    def passwords_match(cls, value, info):
        ...
        if "password" in info.data and value != info.data["password"]:
            raise HTTPException(
                status_code=400, detail="입력한 비밀번호가 일치하지 않습니다."
            )
        return value
...
class ResetPasswordRequest(BaseModel):
    ...
    def validate_password(cls, value):
        ...
        if len(value) < 8:
            raise HTTPException(
                status_code=400, detail="새 비밀번호는 최소 8자리 이상이어야 합니다."
            )
        if (
            not any(c.isupper() for c in value)
            or not any(c.islower() for c in value)
    ...
    def passwords_match(cls, value, info):
        ...
        if "new_password" in info.data and value != info.data["new_password"]:
            raise HTTPException(
                status_code=400, detail="입력한 새 비밀번호가 일치하지 않습니다."
            )
        return value
...
def signup_send_code(request: SignUpRequest):
    ...
    token = create_access_token(
        data={"sub": request.email}, expires_delta=timedelta(minutes=10)
    )
    user_data = {
        "email": request.email,
    ...
    redis_client.setex(
        f"signup_data:{request.email}", timedelta(minutes=10), json.dumps(user_data)
    )
    ...
    return {
        "message": "회원가입 인증 이메일이 발송되었습니다. 10분 이내에 인증을 완료해주세요."
    }
...
def refresh_token(refresh_token: str):
    ...
    if not email:
        raise HTTPException(
            status_code=401, detail="인증이 만료되었습니다. 다시 로그인하세요."
        )
    ...
    new_access_token = create_access_token(
        data={"sub": email, "admin": is_admin_user}, expires_delta=timedelta(hours=12)
    )
...
def findpwd(request: EmailCheckRequest):
    ...
    if not send_verification_email(request.email, code):
        raise HTTPException(
            status_code=500, detail="인증번호 이메일 발송에 실패했습니다."
        )
...
def reset_password(request: ResetPasswordRequest):
    ...
    if not payload or payload.get("action") != "password_reset":
        raise HTTPException(
            status_code=401, detail="인증이 만료되었습니다. 인증번호를 다시 발송하세요."
        )
