...
async def connect(sid, environ):
  ...
  # --- 여기서 토큰을 바로 로그로 남김 ---
  print(f"WebSocket 연결 시도: token='{token}'")
  ...
  # verify_token 내부에서 예외가 날 수 있으니, try-except 구문으로 감싸서 추가 로그를 찍을 수도 있음
  try:
    payload = verify_token(token)  # ✅ JWT 검증
  except Exception as e:
    print(f"❌ WebSocket 인증 실패: 토큰 검증 중 예외 발생 - {e}")
    await sio.emit("error", {"message": "토큰 검증 중 오류가 발생했습니다."}, room=sid)
    await sio.disconnect(sid)
    return
